...
member,context=window,engine;for(engine in selectorEngines)if(Object.prototype.hasOwnProperty.call(selectorEngines,engine)&&window[engine]){members=selectorEngines[engine].replace("*",engine).split(".");member=members.shift();if(member)context=context[member];if(typeof context==="function"){$engine=context;simpleCart.extend(simpleCart.ELEMENT._,selectorFunctions[engine]);return}}},ids:function(){var ids=[];simpleCart.each(function(item){ids.push(item.id())});return ids},save:function(){simpleCart.trigger("beforeSave");
var items={};simpleCart.each(function(item){items[item.id()]=simpleCart.extend(item.fields(),item.options())});localStorage.setItem(namespace+"_items",JSON.stringify(items));simpleCart.trigger("afterSave")},load:function(){sc_items={};var items=localStorage.getItem(namespace+"_items");if(!items)return;try{simpleCart.each(JSON.parse(items),function(item){simpleCart.add(item,true)})}catch(e){simpleCart.error("Error Loading data: "+e)}simpleCart.trigger("load")},ready:function(fn){if(isFunction(fn))if(simpleCart.isReady)fn.call(simpleCart);
else simpleCart.bind("ready",fn);else if(isUndefined(fn)&&!simpleCart.isReady){simpleCart.trigger("ready");simpleCart.isReady=true}},error:function(message){var msg="";if(isString(message))msg=message;else if(isObject(message)&&isString(message.message))msg=message.message;try{console.log("simpleCart(js) Error: "+msg)}catch(e){}simpleCart.trigger("error",[message])}});simpleCart.extend({tax:function(){var totalToTax=settings.taxShipping?simpleCart.total()+simpleCart.shipping():simpleCart.total(),
cost=simpleCart.taxRate()*totalToTax;simpleCart.each(function(item){if(item.get("tax"))cost+=item.get("tax");else if(item.get("taxRate"))cost+=item.get("taxRate")*item.total()});return parseFloat(cost)},taxRate:function(){return settings.taxRate||0},shipping:function(opt_custom_function){if(isFunction(opt_custom_function)){simpleCart({shippingCustom:opt_custom_function});return}var cost=settings.shippingQuantityRate*simpleCart.quantity()+settings.shippingTotalRate*simpleCart.total()+settings.shippingFlatRate;
if(isFunction(settings.shippingCustom))cost+=settings.shippingCustom.call(simpleCart);simpleCart.each(function(item){cost+=parseFloat(item.get("shipping")||0)});return parseFloat(cost)}});cartColumnViews={attr:function(item,column){return item.get(column.attr)||""},currency:function(item,column){return simpleCart.toCurrency(item.get(column.attr)||0)},link:function(item,column){return"<a href='"+item.get(column.attr)+"'>"+column.text+"</a>"},decrement:function(item,column){return"<a href='javascript:;' class='"+
namespace+"_decrement'>"+(column.text||"-")+"</a>"},increment:function(item,column){return"<a href='javascript:;' class='"+namespace+"_increment'>"+(column.text||"+")+"</a>"},image:function(item,column){return"<img src='"+item.get(column.attr)+"'/>"},input:function(item,column){return"<input type='text' value='"+item.get(column.attr)+"' class='"+namespace+"_input'/>"},remove:function(item,column){return"<a href='javascript:;' class='"+namespace+"_remove'>"+(column.text||"X")+"</a>"}};function cartColumn(opts){var options=
opts||{};return simpleCart.extend({attr:"",label:"",view:"attr",text:"",className:"",hide:false},options)}function cartCellView(item,column){var viewFunc=isFunction(column.view)?column.view:isString(column.view)&&isFunction(cartColumnViews[column.view])?cartColumnViews[column.view]:cartColumnViews.attr;return viewFunc.call(simpleCart,item,column)}simpleCart.extend({writeCart:function(selector){var TABLE=settings.cartStyle.toLowerCase(),isTable=TABLE==="table",TR=isTable?"tr":"div",TH=isTable?"th":
"div",TD=isTable?"td":"div",THEAD=isTable?"thead":"div",cart_container=simpleCart.$create(TABLE),thead_container=simpleCart.$create(THEAD),header_container=simpleCart.$create(TR).addClass("headerRow"),container=simpleCart.$(selector),column,klass,label,x,xlen;container.html(" ").append(cart_container);cart_container.append(thead_container);thead_container.append(header_container);for(x=0,xlen=settings.cartColumns.length;x<xlen;x+=1){column=cartColumn(settings.cartColumns[x]);klass="item-"+(column.attr||
column.view||column.label||column.text||"cell")+" "+column.className;label=column.label||"";header_container.append(simpleCart.$create(TH).addClass(klass).html(label))}simpleCart.each(function(item,y){simpleCart.createCartRow(item,y,TR,TD,cart_container)});return cart_container},createCartRow:function(item,y,TR,TD,container){var row=simpleCart.$create(TR).addClass("itemRow row-"+y+" "+(y%2?"even":"odd")).attr("id","cartItem_"+item.id()),j,jlen,column,klass,content,cell;container.append(row);for(j=
0,jlen=settings.cartColumns.length;j<jlen;j+=1){column=cartColumn(settings.cartColumns[j]);klass="item-"+(column.attr||(isString(column.view)?column.view:column.label||column.text||"cell"))+" "+column.className;content=cartCellView(item,column);cell=simpleCart.$create(TD).addClass(klass).html(content);row.append(cell)}return row}});simpleCart.Item=function(info){var _data={},me=this;if(isObject(info))simpleCart.extend(_data,info);item_id+=1;_data.id=_data.id||item_id_namespace+item_id;while(!isUndefined(sc_items[_data.id])){item_id+=
1;_data.id=item_id_namespace+item_id}function checkQuantityAndPrice(){if(isString(_data.price))_data.price=parseFloat(_data.price.replace(simpleCart.currency().decimal,".").replace(/[^0-9\.]+/ig,""));if(isNaN(_data.price))_data.price=0;if(_data.price<0)_data.price=0;if(isString(_data.quantity))_data.quantity=parseInt(_data.quantity.replace(simpleCart.currency().delimiter,""),10);if(isNaN(_data.quantity))_data.quantity=1;if(_data.quantity<=0)me.remove()}me.get=function(name,skipPrototypes){var usePrototypes=
!skipPrototypes;if(isUndefined(name))return name;return isFunction(_data[name])?_data[name].call(me):!isUndefined(_data[name])?_data[name]:isFunction(me[name])&&usePrototypes?me[name].call(me):!isUndefined(me[name])&&usePrototypes?me[name]:_data[name]};me.set=function(name,value){if(!isUndefined(name)){_data[name.toLowerCase()]=value;if(name.toLowerCase()==="price"||name.toLowerCase()==="quantity")checkQuantityAndPrice()}return me};me.equals=function(item){for(var label in _data)if(Object.prototype.hasOwnProperty.call(_data,
label))if(label!=="quantity"&&label!=="id")if(item.get(label)!==_data[label])return false;return true};me.options=function(){var data={};simpleCart.each(_data,function(val,x,label){var add=true;simpleCart.each(me.reservedFields(),function(field){if(field===label)add=false;return add});if(add)data[label]=me.get(label)});return data};checkQuantityAndPrice()};simpleCart.Item._=simpleCart.Item.prototype={increment:function(amount){var diff=amount||1;diff=parseInt(diff,10);this.quantity(this.quantity()+
diff);if(this.quantity()<1){this.remove();return null}return this},decrement:function(amount){var diff=amount||1;return this.increment(-parseInt(diff,10))},remove:function(skipUpdate){var removeItemBool=simpleCart.trigger("beforeRemove",[sc_items[this.id()]]);if(removeItemBool===false)return false;delete sc_items[this.id()];if(!skipUpdate)simpleCart.update();return null},reservedFields:function(){return["quantity","id","item_number","price","name","shipping","tax","taxRate"]},fields:function(){var data=
{},me=this;simpleCart.each(me.reservedFields(),function(field){if(me.get(field))data[field]=me.get(field)});return data},quantity:function(val){return isUndefined(val)?parseInt(this.get("quantity",true)||1,10):this.set("quantity",val)},price:function(val){return isUndefined(val)?parseFloat(this.get("price",true).toString().replace(simpleCart.currency().symbol,"").replace(simpleCart.currency().delimiter,"")||1):this.set("price",parseFloat(val.toString().replace(simpleCart.currency().symbol,"").replace(simpleCart.currency().delimiter,
"")))},id:function(){return this.get("id",false)},total:function(){return this.quantity()*this.price()}};simpleCart.extend({checkout:function(){if(settings.checkout.type.toLowerCase()==="custom"&&isFunction(settings.checkout.fn))settings.checkout.fn.call(simpleCart,settings.checkout);else if(isFunction(simpleCart.checkout[settings.checkout.type])){var checkoutData=simpleCart.checkout[settings.checkout.type].call(simpleCart,settings.checkout);if(checkoutData.data&&checkoutData.action&&checkoutData.method)if(false!==
simpleCart.trigger("beforeCheckout",[checkoutData.data]))simpleCart.generateAndSendForm(checkoutData)}else simpleCart.error("No Valid Checkout Method Specified")},extendCheckout:function(methods){return simpleCart.extend(simpleCart.checkout,methods)},generateAndSendForm:function(opts){var form=simpleCart.$create("form");form.attr("style","display:none;");form.attr("action",opts.action);form.attr("method",opts.method);simpleCart.each(opts.data,function(val,x,name){form.append(simpleCart.$create("input").attr("type",
"hidden").attr("name",name).val(val))});simpleCart.$("body").append(form);form.el.submit();form.remove()}});simpleCart.extendCheckout({PayPal:function(opts){if(!opts.email)return simpleCart.error("No email provided for PayPal checkout");var data={cmd:"_cart",upload:"1",currency_code:simpleCart.currency().code,business:opts.email,rm:opts.method==="GET"?"0":"2",tax_cart:(simpleCart.tax()*1).toFixed(2),handling_cart:(simpleCart.shipping()*1).toFixed(2),charset:"utf-8"},action=opts.sandbox?"https://www.sandbox.paypal.com/cgi-bin/webscr":
"https://www.paypal.com/cgi-bin/webscr",method=opts.method==="GET"?"GET":"POST";if(opts.success)data["return"]=opts.success;if(opts.cancel)data.cancel_return=opts.cancel;if(opts.notify)data.notify_url=opts.notify;simpleCart.each(function(item,x){var counter=x+1,item_options=item.options(),optionCount=0,send;data["item_name_"+counter]=item.get("name");data["quantity_"+counter]=item.quantity();data["amount_"+counter]=(item.price()*1).toFixed(2);data["item_number_"+counter]=item.get("item_number")||
counter;simpleCart.each(item_options,function(val,k,attr){if(k<10){send=true;simpleCart.each(settings.excludeFromCheckout,function(field_name){if(field_name===attr)send=false});if(send){optionCount+=1;data["on"+k+"_"+counter]=attr;data["os"+k+"_"+counter]=val}}});data["option_index_"+x]=Math.min(10,optionCount)});return{action:action,method:method,data:data}},GoogleCheckout:function(opts){if(!opts.merchantID)return simpleCart.error("No merchant id provided for GoogleCheckout");if(simpleCart.currency().code!==
"USD"&&simpleCart.currency().code!=="GBP")return simpleCart.error("Google Checkout only accepts USD and GBP");var data={ship_method_name_1:"Shipping",ship_method_price_1:simpleCart.shipping(),ship_method_currency_1:simpleCart.currency().code,_charset_:""},action="https://checkout.google.com/api/checkout/v2/checkoutForm/Merchant/"+opts.merchantID,method=opts.method==="GET"?"GET":"POST";simpleCart.each(function(item,x){var counter=x+1,options_list=[],send;data["item_name_"+counter]=item.get("name");
data["item_quantity_"+counter]=item.quantity();data["item_price_"+counter]=item.price();data["item_currency_ "+counter]=simpleCart.currency().code;data["item_tax_rate"+counter]=item.get("taxRate")||simpleCart.taxRate();simpleCart.each(item.options(),function(val,x,attr){send=true;simpleCart.each(settings.excludeFromCheckout,function(field_name){if(field_name===attr)send=false});if(send)options_list.push(attr+": "+val)});data["item_description_"+counter]=options_list.join(", ")});return{action:action,
method:method,data:data}},AmazonPayments:function(opts){if(!opts.merchant_signature)return simpleCart.error("No merchant signature provided for Amazon Payments");if(!opts.merchant_id)return simpleCart.error("No merchant id provided for Amazon Payments");if(!opts.aws_access_key_id)return simpleCart.error("No AWS access key id provided for Amazon Payments");var data={aws_access_key_id:opts.aws_access_key_id,merchant_signature:opts.merchant_signature,currency_code:simpleCart.currency().code,tax_rate:simpleCart.taxRate(),
weight_unit:opts.weight_unit||"lb"},action="https://payments"+(opts.sandbox?"-sandbox":"")+".amazon.com/checkout/"+opts.merchant_id,method=opts.method==="GET"?"GET":"POST";simpleCart.each(function(item,x){var counter=x+1,options_list=[];data["item_title_"+counter]=item.get("name");data["item_quantity_"+counter]=item.quantity();data["item_price_"+counter]=item.price();data["item_sku_ "+counter]=item.get("sku")||item.id();data["item_merchant_id_"+counter]=opts.merchant_id;if(item.get("weight"))data["item_weight_"+
counter]=item.get("weight");if(settings.shippingQuantityRate)data["shipping_method_price_per_unit_rate_"+counter]=settings.shippingQuantityRate;simpleCart.each(item.options(),function(val,x,attr){var send=true;simpleCart.each(settings.excludeFromCheckout,function(field_name){if(field_name===attr)send=false});if(send&&attr!=="weight"&&attr!=="tax")options_list.push(attr+": "+val)});data["item_description_"+counter]=options_list.join(", ")});return{action:action,method:method,data:data}},SendForm:function(opts){if(!opts.url)return simpleCart.error("URL required for SendForm Checkout");
var data={currency:simpleCart.currency().code,shipping:simpleCart.shipping(),tax:simpleCart.tax(),taxRate:simpleCart.taxRate(),itemCount:simpleCart.find({}).length},action=opts.url,method=opts.method==="GET"?"GET":"POST";simpleCart.each(function(item,x){var counter=x+1,options_list=[],send;data["item_name_"+counter]=item.get("name");data["item_quantity_"+counter]=item.quantity();data["item_price_"+counter]=item.price();simpleCart.each(item.options(),function(val,x,attr){send=true;simpleCart.each(settings.excludeFromCheckout,
function(field_name){if(field_name===attr)send=false});if(send)options_list.push(attr+": "+val)});data["item_options_"+counter]=options_list.join(", ")});if(opts.success)data["return"]=opts.success;if(opts.cancel)data.cancel_return=opts.cancel;if(opts.extra_data)data=simpleCart.extend(data,opts.extra_data);return{action:action,method:method,data:data}}});eventFunctions={bind:function(name,callback){if(!isFunction(callback))return this;if(!this._events)this._events={};var eventNameList=name.split(/ +/);
simpleCart.each(eventNameList,function(eventName){if(this._events[eventName]===true)callback.apply(this);else if(!isUndefined(this._events[eventName]))this._events[eventName].push(callback);else this._events[eventName]=[callback]});return this},trigger:function(name,options){var returnval=true,x,xlen;if(!this._events)this._events={};if(!isUndefined(this._events[name])&&isFunction(this._events[name][0]))for(x=0,xlen=this._events[name].length;x<xlen;x+=1)returnval=this._events[name][x].apply(this,options||
[]);if(returnval===false)return false;return true}};eventFunctions.on=eventFunctions.bind;simpleCart.extend(eventFunctions);simpleCart.extend(simpleCart.Item._,eventFunctions);baseEvents={beforeAdd:null,afterAdd:null,load:null,beforeSave:null,afterSave:null,update:null,ready:null,checkoutSuccess:null,checkoutFail:null,beforeCheckout:null,beforeRemove:null};simpleCart(baseEvents);simpleCart.each(baseEvents,function(val,x,name){simpleCart.bind(name,function(){if(isFunction(settings[name]))settings[name].apply(this,
arguments)})});simpleCart.extend({toCurrency:function(number,opts){var num=parseFloat(number),opt_input=opts||{},_opts=simpleCart.extend(simpleCart.extend({symbol:"$",decimal:".",delimiter:",",accuracy:2,after:false},simpleCart.currency()),opt_input),numParts=num.toFixed(_opts.accuracy).split("."),dec=numParts[1],ints=numParts[0];ints=simpleCart.chunk(ints.reverse(),3).join(_opts.delimiter.reverse()).reverse();return(!_opts.after?_opts.symbol:"")+ints+(dec?_opts.decimal+dec:"")+(_opts.after?_opts.symbol:
"")},chunk:function(str,n){if(typeof n==="undefined")n=2;var result=str.match(new RegExp(".{1,"+n+"}","g"));return result||[]}});String.prototype.reverse=function(){return this.split("").reverse().join("")};simpleCart.extend({currency:function(currency){if(isString(currency)&&!isUndefined(currencies[currency]))settings.currency=currency;else if(isObject(currency)){currencies[currency.code]=currency;settings.currency=currency.code}else return currencies[settings.currency]}});simpleCart.extend({bindOutlets:function(outlets){simpleCart.each(outlets,
function(callback,x,selector){simpleCart.bind("update",function(){simpleCart.setOutlet("."+namespace+"_"+selector,callback)})})},setOutlet:function(selector,func){var val=func.call(simpleCart,selector);if(isObject(val)&&val.el)simpleCart.$(selector).html(" ").append(val);else if(!isUndefined(val))simpleCart.$(selector).html(val)},bindInputs:function(inputs){simpleCart.each(inputs,function(info){simpleCart.setInput("."+namespace+"_"+info.selector,info.event,info.callback)})},setInput:function(selector,
event,func){simpleCart.$(selector).live(event,func)}});simpleCart.ELEMENT=function(selector){this.create(selector);this.selector=selector||null};simpleCart.extend(selectorFunctions,{"MooTools":{text:function(text){return this.attr(_TEXT_,text)},html:function(html){return this.attr(_HTML_,html)},val:function(val){return this.attr(_VALUE_,val)},attr:function(attr,val){if(isUndefined(val))return this.el[0]&&this.el[0].get(attr);this.el.set(attr,val);return this},remove:function(){this.el.dispose();return null},
addClass:function(klass){this.el.addClass(klass);return this},removeClass:function(klass){this.el.removeClass(klass);return this},append:function(item){this.el.adopt(item.el);return this},each:function(callback){if(isFunction(callback))simpleCart.each(this.el,function(e,i,c){callback.call(i,i,e,c)});return this},click:function(callback){if(isFunction(callback))this.each(function(e){e.addEvent(_CLICK_,function(ev){callback.call(e,ev)})});else if(isUndefined(callback))this.el.fireEvent(_CLICK_);return this},
