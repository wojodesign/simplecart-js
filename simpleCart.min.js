...
a.emailCheckout=function(){};a.customCheckout=function(){};a.load=function(){this.items={};this.quantity=this.total=0;if(readCookie(simpleCart.storagePrefix+"simpleCart_chunks")){for(var a=1*readCookie(simpleCart.storagePrefix+"simpleCart_chunks"),b=[],d="",d=d=0;d<a;d++)b.push(readCookie(simpleCart.storagePrefix+"simpleCart_"+(1+d)));for(var d=unescape(b.join("")),a=d.split("++"),e=0,g=a.length;e<g;e++)b=a[e].split("||"),d=new CartItem,d.parseValuesFromArray(b)&&(d.checkQuantityAndPrice(),this.items[d.id]=
d)}this.isLoaded=!0};a.save=function(){for(var c="",b=[],b=0,b=1*readCookie(simpleCart.storagePrefix+"simpleCart_chunks"),d=0;d<b;d++)eraseCookie(simpleCart.storagePrefix+"simpleCart_"+d);eraseCookie(simpleCart.storagePrefix+"simpleCart_chunks");a.each(function(a){c=c+"++"+a.print()});for(var b=simpleCart.chunk(c.substring(2),simpleCart.MAX_COOKIE_SIZE),d=0,e=b.length;d<e;d++)createCookie(simpleCart.storagePrefix+"simpleCart_"+(1+d),b[d],a.cookieDuration);createCookie(simpleCart.storagePrefix+"simpleCart_chunks",
""+b.length,a.cookieDuration)};a.initializeView=function(){this.totalOutlets=getElementsByClassName("simpleCart_total");this.quantityOutlets=getElementsByClassName("simpleCart_quantity");this.cartDivs=getElementsByClassName("simpleCart_items");this.taxCostOutlets=getElementsByClassName("simpleCart_taxCost");this.taxRateOutlets=getElementsByClassName("simpleCart_taxRate");this.shippingCostOutlets=getElementsByClassName("simpleCart_shippingCost");this.finalTotalOutlets=getElementsByClassName("simpleCart_finalTotal");
