...
d=d+"&item_name_"+b+"="+e.name+"&item_number_"+b+"="+b+"&quantity_"+b+"="+e.quantity+"&amount_"+b+"="+this.currencyStringForPaypalCheckout(e.price)+"&on0_"+b+"=Options&os0_"+b+"="+g;b++}this.shipping()!=0&&(d=d+"&item_name_"+b+"=Shipping&item_number_"+b+"="+b+"&quantity_"+b+"=1&amount_"+b+"="+this.currencyStringForPaypalCheckout(this.shippingCost));this.successUrl&&(d=d+"&return="+this.successUrl);this.cancelUrl&&(d=d+"&cancel_return="+this.cancelUrl);a+=d;window.open(a,"paypal","scrollbars,location,resizable,status")};
a.googleCheckout=function(){if(this.currency!=USD&&this.currency!=GBP)error("Google Checkout only allows the USD and GBP for currency.");else if(this.merchantId===""||this.merchantId===null||!this.merchantId)error("No merchant Id for google checkout supplied.");else{var a=document.createElement("form"),b=1;a.style.display="none";a.method="POST";a.action="https://checkout.google.com/api/checkout/v2/checkoutForm/Merchant/"+this.merchantId;a.acceptCharset="utf-8";for(var d in this.items){var f=this.items[d];
a.appendChild(this.createHiddenElement("item_name_"+b,f.name));a.appendChild(this.createHiddenElement("item_quantity_"+b,f.quantity));a.appendChild(this.createHiddenElement("item_price_"+b,f.price));a.appendChild(this.createHiddenElement("item_currency_"+b,this.currency));a.appendChild(this.createHiddenElement("item_tax_rate_"+b,this.taxRate));a.appendChild(this.createHiddenElement("_charset_",""));var e="",g;for(g in f)typeof f[g]!="function"&&g!="id"&&g!="quantity"&&g!="price"&&(e=e+", "+g+": "+
f[g]);e=e.substring(1);a.appendChild(this.createHiddenElement("item_description_"+b,e));b++}document.body.appendChild(a);a.submit();document.body.removeChild(a)}};a.emailCheckout=function(){};a.customCheckout=function(){};a.load=function(){this.items={};this.quantity=this.total=0;if(readCookie("simpleCart"))for(var a=unescape(readCookie("simpleCart")).split("++"),b=0,d=a.length;b<d;b++){var f=a[b].split("||"),e=new CartItem;e.parseValuesFromArray(f)&&(e.checkQuantityAndPrice(),this.items[e.id]=e)}for(var g in this.items);
this.isLoaded=!0};a.save=function(){var a="",b;for(b in this.items)a=a+"++"+this.items[b].print();createCookie("simpleCart",a.substring(2),this.cookieDuration)};a.initializeView=function(){this.totalOutlets=getElementsByClassName("simpleCart_total");this.quantityOutlets=getElementsByClassName("simpleCart_quantity");this.cartDivs=getElementsByClassName("simpleCart_items");this.taxCostOutlets=getElementsByClassName("simpleCart_taxCost");this.taxRateOutlets=getElementsByClassName("simpleCart_taxRate");
this.shippingCostOutlets=getElementsByClassName("simpleCart_shippingCost");this.finalTotalOutlets=getElementsByClassName("simpleCart_finalTotal");this.addEventToArray(getElementsByClassName("simpleCart_checkout"),simpleCart.checkout,"click");this.addEventToArray(getElementsByClassName("simpleCart_empty"),simpleCart.empty,"click");this.Shelf.readPage();this.pageIsReady=!0};a.updateView=function(){a.updateViewTotals();a.cartDivs&&a.cartDivs.length>0&&a.updateCartView()};a.updateViewTotals=function(){for(var c=
[["quantity","none"],["total","currency"],["shippingCost","currency"],["taxCost","currency"],["taxRate","percentage"],["finalTotal","currency"]],b=0,d=c.length;b<d;b++){var f=c[b][0]+"Outlets",e,g;for(g in a[f]){switch(c[b][1]){case "none":e=""+a[c[b][0]];break;case "currency":e=a.valueToCurrencyString(a[c[b][0]]);break;case "percentage":e=a.valueToPercentageString(a[c[b][0]]);break;default:e=""+a[c[b][0]]}a[f][g].innerHTML=""+e}}};a.updateCartView=function(){var c=[],b,d,f,e,g,h,i,j;d=document.createElement("div");
for(g in a.cartHeaders){h=document.createElement("div");f=a.cartHeaders[g].split("_");h.innerHTML=a.print(f[0]);h.className="item"+f[0];b=1;for(xlen=f.length;b<xlen;b++)if(f[b].toLowerCase()=="noheader")h.style.display="none";d.appendChild(h)}d.className="cartHeaders";c[0]=d;b=1;for(e in a.items){d=document.createElement("div");f=a.items[e];for(g in a.cartHeaders)h=document.createElement("div"),i=a.cartHeaders[g].split("_"),j=a.createCartRow(i,f,j),h.innerHTML=j,h.className="item"+i[0],d.appendChild(h);
d.className="itemContainer";c[b]=d;b++}for(e in a.cartDivs){for(b=a.cartDivs[e];b.childNodes[0];)b.removeChild(b.childNodes[0]);d=0;for(g=c.length;d<g;d++)b.appendChild(c[d])}};a.createCartRow=function(c,b,d){switch(c[0].toLowerCase()){case "total":d=a.valueToCurrencyString(parseFloat(b.price)*parseInt(b.quantity,10));break;case "increment":d=a.valueToLink("+","javascript:;","onclick=\"simpleCart.items['"+b.id+"'].increment();\"");break;case "decrement":d=a.valueToLink("-","javascript:;","onclick=\"simpleCart.items['"+
