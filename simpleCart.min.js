...
function Cart(){var a=this;a.nextId=1;a.Version="2.1";a.Shelf=new Shelf;a.items={};a.isLoaded=!1;a.pageIsReady=!1;a.quantity=0;a.total=0;a.taxRate=0;a.taxCost=0;a.shippingFlatRate=0;a.shippingTotalRate=0;a.shippingQuantityRate=0;a.shippingRate=0;a.shippingCost=0;a.currency=USD;a.checkoutTo=PayPal;a.email="";a.merchantId="";a.successUrl=null;a.cancelUrl=null;a.cartHeaders=["Name","Price","Quantity","Total"];a.add=function(){this.pageIsReady||(this.initializeView(),this.update());this.isLoaded||(this.load(),
this.update());var a=new CartItem;if(!arguments||arguments.length===0)error("No values passed for item.");else{var b=arguments;arguments[0]&&typeof arguments[0]!="string"&&typeof arguments[0]!="number"&&(b=arguments[0]);a.parseValuesFromArray(b);a.checkQuantityAndPrice();this.hasItem(a)?(b=this.hasItem(a),this.items[b].quantity=parseInt(this.items[b].quantity,10)+parseInt(a.quantity,10)):this.items[a.id]=a;this.update()}};a.remove=function(a){var b={},d;for(d in this.items)d!=a&&(b[d]=this.items[d]);
this.items=b};a.empty=function(){simpleCart.items={};simpleCart.update()};a.find=function(c){if(!c)return null;var b=[],d;for(d in a.items){var f=a.items[d],e=!0,g;for(g in c)if(!f[g]||f[g]!=c[g])e=!1;e&&b.push(a.next)}return b.length==0?null:b};a.checkout=function(){if(simpleCart.quantity===0)error("Cart is empty");else switch(simpleCart.checkoutTo){case PayPal:simpleCart.paypalCheckout();break;case GoogleCheckout:simpleCart.googleCheckout();break;case Email:simpleCart.emailCheckout();break;default:simpleCart.customCheckout()}};
a.paypalCheckout=function(){var a="https://www.paypal.com/cgi-bin/webscr?cmd=_cart&upload=1&business="+this.email+"&currency_code="+this.currency,b=1,d="";this.taxRate&&(a=a+"&tax_cart="+this.currencyStringForPaypalCheckout(this.taxCost));for(var f in this.items){var e=this.items[f],g="",h;for(h in e)typeof e[h]!="function"&&h!="id"&&h!="price"&&h!="quantity"&&h!="name"&&h!="shipping"&&(g=g+", "+h+"="+e[h]);g=g.substring(2);d=d+"&item_name_"+b+"="+e.name+"&item_number_"+b+"="+b+"&quantity_"+b+"="+
e.quantity+"&amount_"+b+"="+this.currencyStringForPaypalCheckout(e.price)+"&on0_"+b+"=Options&os0_"+b+"="+g;b++}this.shipping()!=0&&(d=d+"&item_name_"+b+"=Shipping&item_number_"+b+"="+b+"&quantity_"+b+"=1&amount_"+b+"="+this.currencyStringForPaypalCheckout(this.shippingCost));this.successUrl&&(d=d+"&return="+this.successUrl);this.cancelUrl&&(d=d+"&cancel_return="+this.cancelUrl);a+=d;window.open(a,"paypal","scrollbars,location,resizable,status")};a.googleCheckout=function(){if(this.currency!=USD&&
this.currency!=GBP)error("Google Checkout only allows the USD and GBP for currency.");else if(this.merchantId===""||this.merchantId===null||!this.merchantId)error("No merchant Id for google checkout supplied.");else{var a=document.createElement("form"),b=1;a.style.display="none";a.method="POST";a.action="https://checkout.google.com/api/checkout/v2/checkoutForm/Merchant/"+this.merchantId;a.acceptCharset="utf-8";for(var d in this.items){var f=this.items[d];a.appendChild(this.createHiddenElement("item_name_"+
...
b,e));b++}document.body.appendChild(a);a.submit();document.body.removeChild(a)}};a.emailCheckout=function(){};a.customCheckout=function(){};a.load=function(){this.items={};this.quantity=this.total=0;if(readCookie("simpleCart"))for(var a=unescape(readCookie("simpleCart")).split("++"),b=0,d=a.length;b<d;b++){var f=a[b].split("||"),e=new CartItem;e.parseValuesFromArray(f)&&(e.checkQuantityAndPrice(),this.items[e.id]=e)}for(var g in this.items);this.isLoaded=!0};a.save=function(){var a="",b;for(b in this.items)a=
a+"++"+this.items[b].print();createCookie("simpleCart",a.substring(2),30)};a.initializeView=function(){this.totalOutlets=getElementsByClassName("simpleCart_total");this.quantityOutlets=getElementsByClassName("simpleCart_quantity");this.cartDivs=getElementsByClassName("simpleCart_items");this.taxCostOutlets=getElementsByClassName("simpleCart_taxCost");this.taxRateOutlets=getElementsByClassName("simpleCart_taxRate");this.shippingCostOutlets=getElementsByClassName("simpleCart_shippingCost");this.finalTotalOutlets=
getElementsByClassName("simpleCart_finalTotal");this.addEventToArray(getElementsByClassName("simpleCart_checkout"),simpleCart.checkout,"click");this.addEventToArray(getElementsByClassName("simpleCart_empty"),simpleCart.empty,"click");this.Shelf.readPage();this.pageIsReady=!0};a.updateView=function(){a.updateViewTotals();a.cartDivs&&a.cartDivs.length>0&&a.updateCartView()};a.updateViewTotals=function(){for(var c=[["quantity","none"],["total","currency"],["shippingCost","currency"],["taxCost","currency"],
["taxRate","percentage"],["finalTotal","currency"]],b=0,d=c.length;b<d;b++){var f=c[b][0]+"Outlets",e,g;for(g in a[f]){switch(c[b][1]){case "none":e=""+a[c[b][0]];break;case "currency":e=a.valueToCurrencyString(a[c[b][0]]);break;case "percentage":e=a.valueToPercentageString(a[c[b][0]]);break;default:e=""+a[c[b][0]]}a[f][g].innerHTML=""+e}}};a.updateCartView=function(){var c=[],b,d,f,e,g,h,i,j;d=document.createElement("div");for(g in a.cartHeaders){h=document.createElement("div");f=a.cartHeaders[g].split("_");
h.innerHTML=a.print(f[0]);h.className="item"+f[0];b=1;for(xlen=f.length;b<xlen;b++)if(f[b].toLowerCase()=="noheader")h.style.display="none";d.appendChild(h)}d.className="cartHeaders";c[0]=d;b=1;for(e in a.items){d=document.createElement("div");f=a.items[e];for(g in a.cartHeaders)h=document.createElement("div"),i=a.cartHeaders[g].split("_"),j=a.createCartRow(i,f,j),h.innerHTML=j,h.className="item"+i[0],d.appendChild(h);d.className="itemContainer";c[b]=d;b++}for(e in a.cartDivs){for(b=a.cartDivs[e];b.childNodes[0];)b.removeChild(b.childNodes[0]);
d=0;for(g=c.length;d<g;d++)b.appendChild(c[d])}};a.createCartRow=function(c,b,d){switch(c[0].toLowerCase()){case "total":d=a.valueToCurrencyString(parseFloat(b.price)*parseInt(b.quantity,10));break;case "increment":d=a.valueToLink("+","javascript:;","onclick=\"simpleCart.items['"+b.id+"'].increment();\"");break;case "decrement":d=a.valueToLink("-","javascript:;","onclick=\"simpleCart.items['"+b.id+"'].decrement();\"");break;case "remove":d=a.valueToLink("Remove","javascript:;","onclick=\"simpleCart.items['"+
b.id+"'].remove();\"");break;case "price":d=a.valueToCurrencyString(b[c[0].toLowerCase()]?b[c[0].toLowerCase()]:" ");break;default:d=b[c[0].toLowerCase()]?b[c[0].toLowerCase()]:" "}for(var f=1,e=c.length;f<e;f++)switch(option=c[f].toLowerCase(),option){case "image":case "img":d=a.valueToImageString(d);break;case "input":d=a.valueToTextInput(d,"onchange=\"simpleCart.items['"+b.id+"'].set('"+c[0].toLowerCase()+"' , this.value);\"");break;case "div":case "span":case "h1":case "h2":case "h3":case "h4":case "p":d=
a.valueToElement(option,d,"");break;case "noheader":break;default:error("unkown header option: "+option)}return d};a.addEventToArray=function(a,b,d){for(var f in a){var e=a[f];e.addEventListener?e.addEventListener(d,b,!1):e.attachEvent&&e.attachEvent("on"+d,b)}};a.createHiddenElement=function(a,b){var d=document.createElement("input");d.type="hidden";d.name=a;d.value=b;return d};a.currencySymbol=function(){switch(a.currency){case CHF:return"CHF&nbsp;";case CZK:return"CZK&nbsp;";case DKK:return"DKK&nbsp;";
case HUF:return"HUF&nbsp;";case NOK:return"NOK&nbsp;";case PLN:return"PLN&nbsp;";case SEK:return"SEK&nbsp;";case JPY:return"&yen;";case EUR:return"&euro;";case GBP:return"&pound;";case CHF:return"CHF&nbsp;";case USD:case CAD:case AUD:case NZD:case HKD:case SGD:return"&#36;";default:return""}};a.currencyStringForPaypalCheckout=function(c){return a.currencySymbol()=="&#36;"?"$"+parseFloat(c).toFixed(2):""+parseFloat(c).toFixed(2)};a.valueToCurrencyString=function(c){c=parseFloat(c);isNaN(c)&&(c=0);
return c.toCurrency(a.currencySymbol())};a.valueToPercentageString=function(a){return parseFloat(100*a)+"%"};a.valueToImageString=function(a){return a.match(/<\s*img.*src\=/)?a:'<img src="'+a+'" />'};a.valueToTextInput=function(a,b){return'<input type="text" value="'+a+'" '+b+" />"};a.valueToLink=function(a,b,d){return'<a href="'+b+'" '+d+" >"+a+"</a>"};a.valueToElement=function(a,b,d){return"<"+a+" "+d+" > "+b+"</"+a+">"};a.hasItem=function(c){for(var b in a.items){var d=a.items[b],f=!0,e;for(e in c)typeof c[e]!=
"function"&&e!="quantity"&&e!="id"&&c[e]!=d[e]&&(f=!1);if(f)return b}return!1};a.ln={en_us:{quantity:"Quantity",price:"Price",total:"Total",decrement:"Decrement",increment:"Increment",remove:"Remove",tax:"Tax",shipping:"Shipping",image:"Image"}};a.language="en_us";a.print=function(a){return this.ln[this.language]&&this.ln[this.language][a.toLowerCase()]?this.ln[this.language][a.toLowerCase()]:a};a.update=function(){simpleCart.isLoaded||simpleCart.load();simpleCart.pageIsReady||simpleCart.initializeView();
...
var getElementsByClassName=function(a,c,b){getElementsByClassName=document.getElementsByClassName?function(a,b,c){for(var a=(c||document).getElementsByClassName(a),b=b?RegExp("\\b"+b+"\\b","i"):null,c=[],g,h=0,i=a.length;h<i;h+=1)g=a[h],(!b||b.test(g.nodeName))&&c.push(g);return c}:document.evaluate?function(a,b,c){for(var b=b||"*",c=c||document,g=a.split(" "),h="",i=document.documentElement.namespaceURI==="http://www.w3.org/1999/xhtml"?"http://www.w3.org/1999/xhtml":null,a=[],j,k=0,l=g.length;k<
l;k+=1)h+="[contains(concat(' ', @class, ' '), ' "+g[k]+" ')]";try{j=document.evaluate(".//"+b+h,c,i,0,null)}catch(m){j=document.evaluate(".//"+b+h,c,null,0,null)}for(;b=j.iterateNext();)a.push(b);return a}:function(a,b,c){var b=b||"*",c=c||document,g=a.split(" "),a=[],b=b==="*"&&c.all?c.all:c.getElementsByTagName(b),c=[],h;h=0;for(var i=g.length;h<i;h+=1)a.push(RegExp("(^|\\s)"+g[h]+"(\\s|$)"));for(var i=0,j=b.length;i<j;i+=1){g=b[i];h=!1;for(var k=0,l=a.length;k<l;k+=1)if(h=a[k].test(g.className),
